...
class CriticTokenizer:
    ...
    def __init__(self, config: CriticTokenizerConfig):
    ...
    def __call__(self, batch: dict[str, list[int | str]]) -> Tuple[Tensor, dict[Any]]:

        rewards = [batch['rewards']]
        prompts = [observation + '\n' + info for observation, info in zip(
                       batch['observations'], batch['infos'])]
        inputs = self.tokenize(prompts)

        return rewards, inputs
